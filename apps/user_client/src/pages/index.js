...
const ONBOARDING_FEATURES = [
  { href: "/goals", title: "定向", gua: "乾", detail: "把想完成的事变成清楚的主方向。", action: "先说清楚你为什么出发。" },
  { href: "/plan", title: "行旅", gua: "艮", detail: "把长期目标折成今天走得完的一小段。", action: "每天只接住下一处营地。" },
  { href: "/study", title: "攀登", gua: "离", detail: "进入专注山段，并留下真实的学习证据。", action: "完成，而不是只勾选完成。" },
  { href: "/review", title: "回望", gua: "坎", detail: "根据证据找出问题，再生成下一步行动。", action: "让复盘改变明天的路线。" },
  { href: "/knowledge", title: "知识库", gua: "巽", detail: "把理解、错题和输出连接成个人脉络。", action: "让走过的路留下连接。" },
  { href: "/assistant", title: "引路", gua: "兑", detail: "卡住时向你的书鼎提问，获得适合你的提示。", action: "书鼎会按自己的方式陪你想。" },
];
...
function onboardingProfileStep(state) {
  ...
  return `<section class="onboarding-panel onboarding-profile-step"><div class="onboarding-panel__heading"><span class="section-kicker">第一道山门 · 认识自己</span><h2>告诉我你现在<br><em>想走向哪里。</em></h2><p>只收集会影响路线的资料，学校和专业都可以以后再补。</p></div><form class="onboarding-form" data-demo-form="onboarding-profile"><div class="onboarding-form__grid"><label>行者名<input name="name" type="text" value="${esc(profile.name ?? "")}" placeholder="例如：林默" required></label><label>当前阶段<select name="stage" required>${stages.map((stage) => `<option value="${stage}" ${stage === profile.stage ? "selected" : ""}>${stage}</option>`).join("")}</select></label><label>学校 <small>可跳过</small><input name="school" type="text" value="${esc(profile.school ?? "")}" placeholder="例如：某某大学"></label><label>专业 <small>可跳过</small><input name="major" type="text" value="${esc(profile.major ?? "")}" placeholder="例如：计算机科学"></label></div><div class="onboarding-form__section"><span class="onboarding-form__label">此刻最重要的方向</span><input type="hidden" name="goal" data-onboarding-goal value="${selectedGoal?.id ?? ""}"><div class="goal-options onboarding-goal-options">${state.goals.map((goal) => `<button class="goal-option ${goal.id === selectedGoal?.id ? "is-selected" : ""}" type="button" data-goal="${goal.id}" aria-pressed="${goal.id === selectedGoal?.id}"><span class="goal-option__icon">${goal.icon}</span><span><strong>${goal.title}</strong><small>${goal.detail}</small></span><i>${goal.id === selectedGoal?.id ? icon("check", "已选择") : icon("arrow", "选择")}</i></button>`).join("")}</div></div><div class="onboarding-form__footer"><label>每天愿意留出的时间<select name="dailyMinutes">${times.map((time) => `<option value="${time}" ${time === String(profile.dailyMinutes ?? "25") ? "selected" : ""}>${time} 分钟</option>`).join("")}</select></label><button class="button button--primary" type="submit">保存资料，继续 ${icon("arrow")}</button></div></form></section>`;
}
...
function onboardingFeatureStep(state) {
  ...
  return `<section class="onboarding-panel onboarding-feature-step"><div class="onboarding-panel__heading"><span class="section-kicker">第三道山门 · 熟悉山路</span><h2>${selected.name}会带你<br><em>认识六个方向。</em></h2><p>${selected.opening}</p></div><div class="onboarding-feature-layout"><div class="onboarding-feature-companion"><span class="onboarding-feature-companion__mark">${selected.mark}</span><strong>${selected.name}</strong><small>专属教学方式 · ${selected.teachingStyle}</small></div><div class="onboarding-feature-current"><div class="onboarding-feature-current__top"><span>${feature.gua} · 0${featureIndex + 1}</span><span>六方初识</span></div><h3>${feature.title}</h3><p>${feature.detail}</p><strong>${feature.action}</strong>${action(`进入${feature.title}章节`, feature.href, "button button--ink")}</div></div><div class="onboarding-feature-list">${ONBOARDING_FEATURES.map((item, index) => `<a href="${item.href}" data-route="${item.href}" class="onboarding-feature-item ${index === featureIndex ? "is-selected" : ""}" data-feature-index="${index}" aria-current="${index === featureIndex ? "page" : "false"}" title="进入${item.title}章节"><span>${item.gua}</span><strong>${item.title}</strong></a>`).join("")}</div><div class="onboarding-actions"><button class="button button--outline" type="button" data-action="onboarding-back">返回选择书鼎</button><button class="button button--primary" type="button" data-action="onboarding-feature-next" data-feature-route="${feature.href}">${featureIndex === ONBOARDING_FEATURES.length - 1 ? "完成认识，进入我的山门" : `进入${feature.title}章节`} ${icon("arrow")}</button></div></section>`;
}
...
function pageOnboarding(state) {
  ...
  return `<div class="page page--onboarding" data-demo-state="true" style="--onboarding-image: url('${background}')"><div class="onboarding-head"><div><span class="page-intro__kicker">入山引导 · 三道山门</span><h1>让砺境先认识你，<br><em>再陪你走路。</em></h1></div>${demoNote()}</div>${onboardingProgress(step)}${step === 1 ? onboardingProfileStep(state) : step === 2 ? onboardingGuideStep(state) : onboardingFeatureStep(state)}</div>`;
}
