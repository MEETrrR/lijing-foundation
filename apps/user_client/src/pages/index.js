...
function evidenceReview(state) {
  ...
  return `<section class="evidence-review"><div class="section-title"><div><span>行动型 AI 复盘</span><h2>下一步不是一句鼓励，<br><em>而是一件明天能做的事。</em></h2></div><span class="evidence-review__status">${status}</span></div><div class="evidence-review__grid"><div><span>用了什么证据</span><strong>${evidence}</strong></div><div><span>发现了什么问题</span><strong>${esc(review.problem)}</strong></div><div><span>为什么这样判断</span><strong>${esc(review.reason)}</strong></div><div class="evidence-review__next"><span>明日行动</span><strong>${esc(review.nextAction)}</strong></div></div></section>`;
}
