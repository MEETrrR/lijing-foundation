...
function pageOnboarding(state) {
  return `<div class="page page--onboarding" data-demo-state="${state.isDemo}"><div class="onboarding-head"><div><span class="page-intro__kicker">入山引导 · 一次填写</span><h1>先留下当前材料，<br><em>再拿到今天这一步。</em></h1></div><div class="onboarding-head__aside">${demoNote(state)}<span class="onboarding-nav-note">约 1 分钟 · 可随时修改</span></div></div>${onboardingProfileStep(state)}</div>`;
}
...
function materialStudyPage(state) {
  ...
  if (!currentAction) {
    return `<div class="page page--study page--study-material" data-demo-state="${state.isDemo}">${intro("/study", state, "材料驱动 · 先把卡住的地方交给器灵")}${persistenceNotice(state)}<div class="study-layout"><section class="study-focus study-focus--material"><div class="study-focus__top"><span class="section-kicker">${companion.screenState === "need_material" ? "等待你的材料" : "材料入口"}</span>${demoNote(state)}<span class="study-focus__timer">文字或照片</span></div><div class="study-focus__title"><span class="study-focus__gua">器</span><h2>先把正在卡住的地方交出来</h2><p>可以是一道题、一段笔记、作答草稿或错因说明。器灵只根据你给出的内容判断，不替你编造错因。</p></div><section class="study-action material-intake"><div class="study-action__heading"><span>第一步 · 留下可检索的材料</span><h3>材料 → 诊断 → 一条现在能完成的行动</h3><p>可以直接粘贴文字，或拍下题目和笔记。确认后的文字会按你的账号隔离，并保留引用位置。</p></div><form class="material-intake__form" data-demo-form="learning-artifact"><div class="material-intake__grid"><label>材料标题<input name="source_title" type="text" maxlength="160" placeholder="例如：极限题 12 · 洛必达条件混淆"></label><label>科目<select name="subject"><option value="数学" selected>数学</option><option value="408">408</option><option value="数学 / 408">数学 / 408</option></select></label><label>材料类型<select name="kind"><option value="question" selected>题目</option><option value="note">笔记</option><option value="attempt_draft">作答草稿</option><option value="answer_reference">错因说明 / 参考答案</option></select></label></div><label>粘贴材料<textarea name="content_text" rows="10" maxlength="12000" placeholder="把题干、你的思路、写到哪一步、哪里不确定一起贴进来……"></textarea></label><label class="material-photo">拍题 / 拍笔记<input name="material_image" data-material-image type="file" accept="image/jpeg,image/png,image/gif" capture="environment"><span data-material-image-name>选择图片或直接拍摄</span></label><p class="material-photo__notice">图片仅用于本次识别，不会保存；识别后请确认文字再交给器灵。</p><p class="material-photo__uncertain" data-material-image-uncertain hidden></p><div class="material-intake__footer"><span>建议包含：题目条件、你的尝试、卡住的位置。</span><button class="button button--ink material-intake__submit" type="submit"><span class="material-submit-label material-submit-label--default">交给器灵</span><span class="material-submit-label material-submit-label--confirm">确认文字并交给器灵</span> ${icon("arrow")}</button></div></form>${artifacts.length ? `<div class="material-library"><div class="material-library__head"><span>已保存材料</span><strong>${artifacts.length} 份</strong></div><div class="material-library__list">${artifacts.slice(0, 4).map((artifact) => `<article><span>${esc(artifact.kind)}</span><strong>${esc(artifact.source_title)}</strong><small>${esc(artifact.subject)} · ${esc(artifact.updated_at ? new Date(artifact.updated_at).toLocaleDateString("zh-CN") : "刚刚")}</small></article>`).join("")}</div></div>` : ""}</section><aside class="study-aside"><section class="study-aside__tip"><span class="study-aside__tip-mark">知</span><div class="study-aside__tip-copy"><span>器灵的边界</span><p>没有材料，就没有诊断；没有证据，就不把一次完成写成掌握。</p><a href="/assistant" data-route="/assistant">先问一个问题 ${icon("arrow")}</a></div></section></aside></div></div>`;
  }
  ...
  const diagnosisBlock = diagnosis
    ? diagnosis.status === "degraded"
      ? `<div class="material-diagnosis material-diagnosis--degraded"><span>基于材料生成的保底行动</span><p>模型诊断暂未完成。这条行动只根据你提交的材料生成，不作掌握、正确率或路线结论。</p>${Array.isArray(diagnosis.unknowns) && diagnosis.unknowns.length ? `<small>仍待验证：${esc(diagnosis.unknowns.join("；"))}</small>` : ""}</div>`
