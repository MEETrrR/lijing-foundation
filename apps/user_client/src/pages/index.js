...
function pagePlan(state) {
  return `<div class="page page--plan" data-demo-state="true">${intro("/plan", state)}<div class="plan-head"><div class="plan-head__progress"><span>${state.pilot.vertical} · 本阶段证据</span><strong>${state.today.completed.toLocaleString("zh-CN")} <small>/ ${state.today.total} 段</small></strong>${progressBar(Math.round((state.today.completed / state.today.total) * 100))}<div><span>目标</span><span>证据</span><span>复测</span></div></div><div class="plan-head__balance"><div class="balance-wheel balance-wheel--large"><span></span><b>☯</b></div><div><span>今日完成</span><strong>${state.today.completed} / ${state.today.total}</strong><p>先提交，再复盘。</p></div></div></div><div class="plan-route"><div class="plan-route__label"><span>DAY 04 / 14</span><strong>试点 · 第四日</strong><small>${state.pilot.subject}</small></div><div class="plan-route__list">${state.today.tasks.map(taskItem).join("")}<div class="route-future"><span class="route-future__line"></span><span class="route-future__cloud">后续任务将在证据提交后解锁</span></div></div></div></div>`;
}
...
function evidenceReview(state) {
  ...
  return `<section class="evidence-review"><div class="section-title"><div><span>行动型 AI 复盘</span><h2>下一步不是一句鼓励，<br><em>而是一件明天能做的事。</em></h2></div><span class="evidence-review__status">${state.pilot.reviewReady ? "已生成" : "演示样例"}</span></div><div class="evidence-review__grid"><div><span>用了什么证据</span><strong>${evidence}</strong></div><div><span>发现了什么问题</span><strong>${review.problem}</strong></div><div><span>为什么这样判断</span><strong>${review.reason}</strong></div><div class="evidence-review__next"><span>明日行动</span><strong>${review.nextAction}</strong></div></div></section>`;
}

function pageStudy(state) {
  const active = state.today.tasks.find((task) => task.status === "active") ?? state.today.tasks.at(-1);
  return `<div class="page page--study" data-demo-state="true">${intro("/study", state)}<div class="study-layout"><section class="study-focus"><div class="study-focus__top"><span class="section-kicker">当前山段 · ${active.type}</span>${demoNote()}<span class="study-focus__timer">25:00</span></div><div class="study-focus__title"><span class="study-focus__gua">${active.gua}</span><h2>${active.title}</h2><p>安静完成学习，结束时提交你能提供的最高等级证据。砺境不会替你宣布“已掌握”。</p></div><div class="study-question"><span class="question-label">今日短测 · 01</span><h3>如果函数在某点连续，它一定在该点可导吗？</h3><div class="answer-lines"><button type="button" data-action="answer" class="answer-line"><span>A</span><span>是，连续性已经包含了可导性</span></button><button type="button" data-action="answer" class="answer-line"><span>B</span><span>不一定，可导性还需要更强的局部条件</span></button><button type="button" data-action="answer" class="answer-line"><span>C</span><span>只有函数值大于零时才可以</span></button></div><button class="button button--ink study-submit" type="button" data-action="submit-answer">记录短测答案 ${icon("arrow")}</button></div><div class="evidence-submit"><div class="section-title"><div><span>完成证据</span><h2>你这次留下了什么？</h2></div><span class="evidence-submit__level">当前 L${state.pilot.selectedEvidenceLevel}</span></div><p>选择最高证据等级，并用一句话记录内容。正式版将由服务端保存并进入复盘。</p><div class="evidence-levels">${state.pilot.evidenceLevels.map((item) => evidenceLevelItem(item, item.level === state.pilot.selectedEvidenceLevel)).join("")}</div><label class="evidence-submit__field">证据摘要<textarea data-evidence-input rows="4" placeholder="例如：我用反例说明连续不推出可导，并记录了卡住的步骤。">${esc(state.pilot.submittedEvidence)}</textarea></label><button class="button button--ink" type="button" data-action="submit-evidence">提交证据并生成复盘 ${icon("arrow")}</button></div><button class="knowledge-capture-link" type="button" data-action="capture-knowledge" data-knowledge-title="${active.title}" data-knowledge-source="攀登 · 当前山段">${icon("plus")} 收录这一段到知识库</button></section><aside class="study-aside"><div class="study-aside__route"><span class="section-kicker">试点记录</span><div class="mini-mountain"><span class="mini-mountain__path"></span><i class="mini-mountain__dot mini-mountain__dot--one"></i><i class="mini-mountain__dot mini-mountain__dot--two"></i><i class="mini-mountain__dot mini-mountain__dot--three"></i></div><div class="study-aside__legend"><span><i class="dot dot--gold"></i>今日任务</span><span><i class="dot dot--gray"></i>证据解锁下一步</span></div></div><div class="study-aside__tip"><span class="study-aside__tip-mark">灯</span><p>卡住时只问一个问题：<br>我能留下哪一种证据？</p><a href="/review" data-route="/review">查看复盘规则 ${icon("arrow")}</a></div></aside></div></div>`;
}

function pageReview(state) {
  return `<div class="page page--review" data-demo-state="true">${intro("/review", state)}<div class="review-top"><div class="review-top__bagua">${renderBaguaField({ active: "坎", label: "今日回望 · 坎位" })}</div><div class="review-top__copy"><span class="section-kicker">${state.pilot.vertical} · 今日回望</span><h2>让走过的路<br><em>变成下一步行动。</em></h2><p>复盘只根据你提交的证据，不把自报完成包装成掌握结论。</p>${action("继续提交证据", "/study", "button button--ink")}</div><div class="review-top__stats">${metaLine("本次证据", `L${state.pilot.selectedEvidenceLevel}`)}${metaLine("复盘状态", state.pilot.reviewReady ? "已生成" : "待提交")}${metaLine("下一动作", "明日 15 分钟")}</div></div>${evidenceReview(state)}<section class="review-list">${sectionTitle("需要你回望的山脊", "证据覆盖情况", action("查看档案", "/knowledge", "text-link", "arrow"))}<div class="knowledge-rows">${state.knowledge.slice(0, 3).map((item) => `<article class="knowledge-row"><span class="knowledge-row__gua">${item.gua}</span><div><span>${item.domain}</span><h3>${item.title}</h3></div><div class="knowledge-row__mastery">${progressBar(item.mastery, item.color === "cinnabar" ? "red" : "amber")}<strong>L${item.evidenceLevel ?? 2} · ${item.mastery}%</strong></div><a href="/study" data-route="/study" aria-label="复习 ${item.title}">${icon("arrow", "复习")}</a></article>`).join("")}</div></section></div>`;
}
...
function pageAssistant(state) {
  ...
  return `<div class="page page--assistant" data-demo-state="true">${intro("/assistant", state)}<div class="assistant-layout"><section class="assistant-dialog"><div class="assistant-dialog__header"><span class="assistant-dialog__seal">${selected.mark}</span><div><span>证据引路 · 只在卡住时使用</span><strong>${selected.name}</strong></div><i class="online-dot"></i><small>根据当前任务和学习记录给出下一步</small></div><div class="assistant-dialog__body"><div class="message message--guide"><span class="message__mark">${selected.mark}</span><p>先把你卡住的地方说清楚。<br>我会帮你找到最小可执行动作。</p></div><div class="suggestion-list"><button type="button" data-action="ask-guide">根据我的错题，明天先补什么？</button><button type="button" data-action="ask-guide">把今天目标拆成一个 25 分钟动作</button><button type="button" data-action="ask-guide">我还没有证据，最小输出是什么？</button></div></div><form class="assistant-composer" data-demo-form="assistant"><input name="prompt" placeholder="说出你卡住的地方……" aria-label="向引路人提问"><button type="submit" class="icon-button icon-button--dark" title="发送问题">${icon("arrow", "发送问题")}</button></form></section><aside class="assistant-aside"><section class="assistant-guide-card assistant-guide-card--${selected.kind}"><div class="assistant-guide-card__art"><img src="${selectedAsset.path}" alt="${selectedAsset.alt}"></div><div class="assistant-guide-card__copy"><span class="section-kicker">视觉层 · 非核心能力</span><strong>${selected.name}</strong><p>${selected.detail}</p></div></section><section class="assistant-guide-picker"><div class="assistant-guide-picker__heading"><span class="section-kicker">选择视觉引路物</span><small>它只改变反馈气质，不改变学习结论。</small></div><div class="guide-options">${guide.options.map((option) => `<button class="guide-option guide-option--${option.kind} ${option.assetId === selected.assetId ? "is-selected" : ""}" type="button" data-action="select-guide" data-guide="${option.assetId}" aria-pressed="${option.assetId === selected.assetId}"><span class="guide-option__art"><img src="${assetUrl(option.assetId)}" alt="${option.name}"></span><span class="guide-option__text"><strong>${option.name}</strong><small>${option.detail}</small></span><span class="guide-option__mark">${option.mark}</span></button>`).join("")}</div></section><div class="assistant-aside__note"><span class="section-kicker">今日提示</span><p>不要先追求完整计划，先留下一个能被回看的输出。</p></div></aside></div></div>`;
}
