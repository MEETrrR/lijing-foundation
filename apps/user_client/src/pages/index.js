...
function pageAssistant(state) {
  ...
  const selected = guide.options.find((option) => option.assetId === guide.selectedAssetId) ?? guide.options[0];
  const selectedAsset = getAsset(selected.assetId);
  const response = state.pilot.assistantResponse
    ? esc(state.pilot.assistantResponse).replace(/\n/g, "<br>")
    : state.pilot.assistantError
      ? `AI 暂时不可用：${esc(state.pilot.assistantError)}`
      : state.isDemo
        ? esc(selected.opening)
        : "AI 尚未接通。当前账号不会把静态文案当作真实回答，请先完成服务端 AI 配置。";
...
function pageProfile(state) {
  ...
  return `<div class="page page--profile" data-demo-state="true">${intro("/profile", state)}<div class="profile-layout"><section class="profile-card"><div class="profile-card__top"><div class="profile-card__seal">行</div><div><span class="section-kicker">行者编号 · ${account ? "REAL" : "DEMO-01"}</span><h2>${esc(state.user.name)}</h2><p>${esc(state.user.title)}</p></div><button class="icon-button" type="button" title="编辑个人资料">${icon("settings", "编辑个人资料")}</button></div><div class="profile-card__path"><span>第一次入山</span><i></i><strong>${profileMilestone}</strong><i></i><span>下一枚印记</span></div><div class="profile-card__footer">${metaLine("已行进", profileDays)}${metaLine("当前高度", `${state.mountain.currentHeight.toLocaleString("zh-CN")}m`)}${metaLine("回望次数", profileReviews)}</div></section><section class="settings-list">${sectionTitle("行者设置", "让这幅长卷适合你", demoNote(state))}<button class="setting-row" type="button" data-action="toggle-motion"><span>${icon("spark")}</span><div><strong>动效与云雾</strong><small>保留沉浸感，减少不必要的运动</small></div><i class="toggle is-on"><b></b></i></button><button class="setting-row" type="button"><span>${icon("bell")}</span><div><strong>通知与提醒</strong><small>决定什么时候收到下一处营地的消息</small></div>${icon("chevron")}</button><button class="setting-row" type="button"><span>${icon("user")}</span><div><strong>隐私与账号</strong><small>管理你的身份印记和数据边界</small></div>${icon("chevron")}</button><button class="setting-row setting-row--danger" type="button" data-action="logout"><span>${icon("arrow", "退出")}</span><div><strong>退出山门</strong><small>结束当前设备上的登录会话</small></div>${icon("chevron")}</button></section></div></div>`;
}
