...
export function renderTopbar(meta, state, currentRoute = "") {
  ...
  return `<header class="topbar" data-tour-target="topbar"><div class="topbar__chapter"><span class="topbar__chapter-mark">${meta.gua}</span><div><span class="sr-only">当前章节</span><span class="topbar__eyebrow">${meta.eyebrow}</span><span class="topbar__title">${meta.chapter}</span></div></div><div class="topbar__right"><span class="connection"><i></i>${connectionLabel}</span>${tourAction}<a class="topbar__icon" href="/profile" data-route="/profile" aria-label="通知">${icon("bell", "通知")}</a><a class="topbar__profile" href="/profile" data-route="/profile"><span class="avatar-dot">行</span><span>${state.user.name}</span>${icon("chevron")}</a></div></header>`;
}
