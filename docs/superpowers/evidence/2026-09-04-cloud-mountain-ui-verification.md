...
当前实现通过 Node 原生静态服务提供 `http://127.0.0.1:4187`，并覆盖以下 16 个路由：
...
- 主题：高级东方未来与云海登山叙事，页面内容围绕“每次自律攀爬”展开。
- 主色：深墨蓝黑、岩灰、琥珀金，少量朱砂红作为状态强调。
- 场景：入口使用山路与行者画面，内页使用云海山门场景；导航使用八方卦位、太极与星轨结构。
...
- 客户端使用静态 `DEMO_STATE`，所有用户、山路、目标、知识、成就和地图数据均为演示内容。
...
- 目标选择：`goal-skill` 获得 `is-selected` 与 `aria-pressed="true"`。
