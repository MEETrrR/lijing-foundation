...
- 认证表单仍是演示态；助手和证据复盘默认不会调用 Provider。若本机显式配置 `AI_ENABLED=true`、模型和服务端密钥，才会通过本地私有适配器调用 `/api/v1/ai/assist` 或 `/api/v1/ai/review`；该适配器不等于正式 AI Gateway，也不提供生产级账号配额、审计和持久化。
...
本轮使用最新工作树在隔离端口 `4191` 复跑：默认 `AI_ENABLED=false` 时，健康接口返回 `200` 且 `ai_configured=false`；提交证据后进入 `/review` 并显示“未完成”，引路页显示明确的 AI 未配置降级；浏览器控制台、页面错误和请求失败均为 0。

## 本地 AI 适配器边界

- 默认 `AI_ENABLED=false`，未配置 Provider 时接口返回明确的 `503 ai_not_configured`，不会伪造成功结果。
- 健康检查只返回是否配置，不返回密钥；请求体有大小限制，AI 调用有本机内存中的 IP 日额度。
- 以上控制只用于本机私有试点，不能替代生产环境的身份、配额账本、幂等、审计、内容安全、成本结算和 kill switch。
...
- `pnpm client:test`：18/18 通过。
- `pnpm client:ai:test`：1/1 通过，Provider mock 调用只发生在服务端，返回结构化复盘且不泄露测试密钥。
