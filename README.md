...
- **可运行用户端：** 用户端已经有统一的东方水墨世界壳层、八卦功能目录、页面转场、个人知识脉络、学习证据试点、回望复盘和引路灵器选择。
...
当前纵向试点内容包含 **考研数学二 · 高等数学 · 极限与连续**。未登录用户不能进入业务页面；登录后只读取和保存自己的服务端状态，不会继承样例进度。
...
本地开发/测试启动模式（会先关闭旧的砺境客户端/API 进程，修改源码后自动重启）：
...
默认访问 `http://127.0.0.1:4187`；端口被占用时运行 `$env:PORT=4190; npm run dev`。
...
生产模式同样会先关闭旧进程，并从当前环境或 `.env.production` 读取配置；至少需要 `SUPABASE_DATABASE_URL`，否则会直接停止并提示配置错误。
...
- `DEMO_STATE` 仅作为页面渲染测试夹具保留，运行时没有演示登录入口；未登录访问会停留在注册/登录页。
- 登录用户的资料、目标、引路器选择、今日任务状态、学习证据/复盘和个人知识节点通过 `/api/v1/me/state` 按用户保存；身份、学习结算、长期记忆和 AI Gateway 也由服务端数据库适配器保存。未配置 `SUPABASE_DATABASE_URL` 时，本地服务才会回退到开发用内存适配器。
- 新账号从空白状态开始；没有服务端任务或 AI 结果时，客户端不会用合成进度或本地复盘冒充真实结果。
